...
def _scan_main_start_menu_apps() -> List[Dict[str, str]]:
    ...
    # COM（WScript.Shell）在本线程初始化一次并复用，避免每个 .lnk 都新建 COM 对象
    with com_initialized():
        ...
        try:
            ...
            logger.debug(f"[WindowsScanner] 创建 WScript.Shell 失败: {e}")
...
def _scan_main_registry_apps() -> List[Dict[str, str]]:
    ...
    try:
        # 同时查询 HKLM（64位/32位WOW6432Node）与 HKCU（用户级安装），
        # 并强制 UTF-8 输出以正确解码中文应用名。
        powershell_cmd = [
            "powershell",
            "-NoProfile",
            "-Command",
...
def _resolve_shortcut_target(shortcut_path: str, shell=None) -> Optional[str]:
    """解析Windows快捷方式的目标路径.

    Args:
        shortcut_path: 快捷方式文件路径
        shell: 可选，已创建的 WScript.Shell COM 对象（须在已初始化 COM 的线程内）。
            传入可复用以避免为每个快捷方式新建 COM 对象。
    ...
    try:
        if shell is None:
            # 未提供复用对象时，在本线程自行初始化 COM
            with com_initialized():
                import win32com.client

                shell = win32com.client.Dispatch("WScript.Shell")
                return _read_shortcut_target(shell, shortcut_path)
        return _read_shortcut_target(shell, shortcut_path)
...
def _read_shortcut_target(shell, shortcut_path: str) -> Optional[str]:
    """从已有的 WScript.Shell 对象读取快捷方式目标."""
