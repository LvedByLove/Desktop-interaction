...
from .tools import get_brightness, get_volume, set_brightness, set_volume
...
class SystemToolsManager:
    ...
    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        ...
        try:
            ...
            # 注册屏幕亮度控制工具
            self._register_brightness_control_tool(
                add_tool, PropertyList, Property, PropertyType
            )

            # 注册屏幕亮度获取工具
            self._register_brightness_get_tool(
                add_tool, PropertyList, Property, PropertyType
            )
    ...
    def _register_brightness_control_tool(
        self, add_tool, PropertyList, Property, PropertyType
    ):
        """
        注册屏幕亮度控制工具.
        ...
        add_tool(
            (
                ...
                "Set the system display/screen brightness to an absolute value (0-100).\n"
                "Use when user mentions: brightness, screen brightness, display brightness, brighter, "
                "darker, dim, reduce brightness, increase brightness.\n"
                "Examples: 'set brightness to 60', 'make the screen brighter', 'dim the screen', "
                "'turn brightness down', '亮度设为60', '把屏幕亮度调到60', '屏幕亮一点', "
                "'屏幕太亮了调暗一点', '降低亮度', '提高屏幕亮度'.\n"
                "Parameter:\n"
                "- brightness: Integer (0-100) representing the target screen/display brightness level.",
                brightness_props,
                set_brightness,
            )
        )
        logger.debug("[SystemManager] 注册屏幕亮度控制工具成功")

    def _register_brightness_get_tool(
        self, add_tool, PropertyList, Property, PropertyType
    ):
        """
        注册屏幕亮度获取工具.
        ...
        add_tool(
            (
                "self.display.get_brightness",
                "Get the current system display/screen brightness level.\n"
                "Use when user asks about: current brightness, screen brightness level, "
                "how bright the screen is.\n"
                "Examples: 'what is the current brightness?', 'check screen brightness', "
                "'how bright is the display?', '现在亮度多少?', '查看屏幕亮度', "
                "'屏幕亮度是多少'.\n"
                "Returns:\n"
                "- Integer (0-100) representing the current display/screen brightness level.",
                get_brightness_props,
                get_brightness,
            )
        )
        logger.debug("[SystemManager] 注册屏幕亮度获取工具成功")
