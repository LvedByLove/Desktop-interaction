...
class MusicToolsManager:
    ...
    def init_tools(self, add_tool, PropertyList, Property, PropertyType):
        ...
        try:
            ...
            # 注册搜索工具
            self._register_search_tool(add_tool, PropertyList, Property, PropertyType)
    ...
    def _register_search_tool(self, add_tool, PropertyList, Property, PropertyType):
        """
        注册搜索工具.
        """
        ...
        async def search_wrapper(args: Dict[str, Any]) -> str:
            ...
            result = await self._music_player.search_music(query)
            return result.get("message", "搜索完成")

        search_props = PropertyList([Property("query", PropertyType.STRING)])

        add_tool(
            (
                "music_player.search",
                "搜索在线曲库但不播放。用于用户只想查找歌曲、查看可播放版本、询问有没有某首歌时。"
                "参数 query 可以是歌名、歌手名或歌词关键词，例如'晴天'、'周杰伦 稻香'。",
                search_props,
                search_wrapper,
            )
        )
        logger.debug("[MusicManager] 注册音乐搜索工具成功")
    ...
    def _register_search_and_play_tool(
        ...
        add_tool(
            (
                ...
                "搜索并播放指定的歌曲。根据歌名在线搜索歌曲，下载到本地缓存后自动开始播放。"
                "如果已有音乐在播放，会自动停止当前音乐并播放新歌曲。"
                "用于播放用户请求的特定歌曲，例如'播放周杰伦的稻香'、'听一下孤勇者'。"
                "如果用户只是想查询有哪些歌，不要调用本工具，应调用 music_player.search。",
                search_props,
