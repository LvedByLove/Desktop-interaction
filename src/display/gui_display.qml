...
Rectangle {
    ...
    signal musicIndicatorDragStart()
    signal musicIndicatorDragMove()
    signal musicIndicatorDragEnd(bool moved)
    ...
    property bool musicIndicatorDragging: false
    property bool musicIndicatorMoved: false
    property real musicIndicatorPressX: 0
    property real musicIndicatorPressY: 0

    // 窗口尺寸状态
    property int normalWidth: 36
    property int normalHeight: 36
    // 隐藏模式：完全不显示（收缩为 1x1 透明区域，不再是黑线）
    property int hiddenWidth: 1
    ...
    property int musicIndicatorWidth: 38
    property int musicIndicatorHeight: 30
    ...
    function refreshCurrentModeSize() {
        if (viewMode !== "tts") return
        root.radius = modeRadius(viewMode)
    ...
    Timer {
        ...
        onTriggered: {
            if (!hoverArea.containsMouse && viewMode !== "hidden" && viewMode !== "tts" && viewMode !== "input") {
                // 隐藏模式完全不显示，且不再允许通过鼠标悬停退出
                enterHiddenMode(false)
            }
        }
    }
    ...
    RowLayout {
        ...
        anchors.fill: parent
        anchors.leftMargin: collapsed ? 9 : 14
        anchors.rightMargin: collapsed ? 9 : 14
        anchors.topMargin: collapsed ? 3 : 4
        anchors.bottomMargin: collapsed ? 3 : 4
        ...
        Item {
            ...
            Text {
                ...
                // 会导致滚动判断 (ttsText.width > containerWidth) 永远为 false，
                // 长文本无法滚动而被裁剪。
                width: implicitWidth
        ...
        Timer {
            ...
            onTriggered: {
                ...
                if (textWidth > textContainer.width) {
                    scrollTimer.start()
                }
            }
        }
        ...
        Row {
            spacing: ttsContent.collapsed ? 3 : 4
            Layout.alignment: Qt.AlignVCenter
            Layout.preferredWidth: ttsContent.collapsed ? 20 : 24

            Rectangle {
                width: ttsContent.collapsed ? 3 : 4
                height: ttsContent.collapsed ? 12 : 16
                radius: width / 2
                color: "#52b7ff"
                SequentialAnimation on height {
                    loops: Animation.Infinite
                    NumberAnimation { from: ttsContent.collapsed ? 6 : 8; to: ttsContent.collapsed ? 12 : 16; duration: 500; easing.type: Easing.InOutQuad }
                    NumberAnimation { from: ttsContent.collapsed ? 12 : 16; to: ttsContent.collapsed ? 6 : 8; duration: 500; easing.type: Easing.InOutQuad }
                }
            }
            Rectangle {
                width: ttsContent.collapsed ? 3 : 4
    ...
    MouseArea {
        id: musicIndicatorDragArea
        anchors.fill: parent
        visible: viewMode === "tts" && root.isMusicLyricsCollapsed()
        enabled: visible
        hoverEnabled: true
        ...
        onPressed: function(mouse) {
            if (mouse.button !== Qt.LeftButton) return
            musicIndicatorDragging = true
            musicIndicatorMoved = false
            musicIndicatorPressX = mouse.x
            musicIndicatorPressY = mouse.y
            cursorShape = Qt.ClosedHandCursor
            root.musicIndicatorDragStart()
        }

        onPositionChanged: function(mouse) {
            if (!musicIndicatorDragging) return
            if (Math.abs(mouse.x - musicIndicatorPressX) > 3 || Math.abs(mouse.y - musicIndicatorPressY) > 3) {
                musicIndicatorMoved = true
            }
            root.musicIndicatorDragMove()
        }

        onReleased: function(mouse) {
            if (!musicIndicatorDragging) return
            musicIndicatorDragging = false
            cursorShape = Qt.OpenHandCursor
            root.musicIndicatorDragEnd(musicIndicatorMoved)
        }
        ...
        onClicked: function(mouse) {
            if (musicIndicatorMoved) {
                mouse.accepted = true
                return
            }
            if (root.toggleMusicLyricsCollapsed()) {
                mouse.accepted = true
            }
        }
    }
