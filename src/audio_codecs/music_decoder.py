...
class MusicDecoder:
    ...
    # ffmpeg 可执行路径的进程级缓存：探测一次后所有实例复用
    # （MusicDecoder 每次播放都会新建实例，若不缓存则每首歌都要重新探测多条路径）
    ...
    def _subprocess_startup_options(self) -> dict:
        """
    ...
    def _find_ffmpeg(self) -> str:
        ...
        startup_options = self._subprocess_startup_options()
        for path in paths:
            try:
    ...
    async def _resolve_ffmpeg_path(self) -> str:
        """
        惰性解析 ffmpeg 路径并按进程缓存。

        探测本身是阻塞的 subprocess 调用，放到线程中执行以免卡住事件循环。
        """
        if MusicDecoder._cached_ffmpeg_path is None:
            MusicDecoder._cached_ffmpeg_path = await asyncio.to_thread(
                self._find_ffmpeg
            )
            logger.info(f"已解析 FFmpeg 路径: {MusicDecoder._cached_ffmpeg_path}")
        return MusicDecoder._cached_ffmpeg_path
    ...
    async def start_decode(
        ...
        try:
            ffmpeg_path = await self._resolve_ffmpeg_path()
            ...
            self._process = await asyncio.create_subprocess_exec(
                ...
                **self._subprocess_startup_options(),
            )
