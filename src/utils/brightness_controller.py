...
class BrightnessController:
    ...
    def _init_windows(self) -> None:
        ...
        try:
            # 仅验证 wmi 模块可导入；连接在使用时于工作线程内创建
            # （COM 对象具有线程亲和性，不能在初始化线程创建后跨线程使用）
            self._lazy_import("wmi")
            self.wmi_c = None
    ...
    def _get_wmi_connection(self):
        """在当前线程（已初始化 COM）内创建并返回 WMI 连接.

        COM/WMI 对象不能跨 apartment 复用，因此连接在 com_initialized 块内
        按需创建并在同一块内使用完毕。
        """
    ...
    def _get_windows_brightness(self) -> Callable[[], int]:
        ...
        def get_brightness():
            with com_initialized():
                ...
                wmi_c = self._get_wmi_connection()
                for monitor in wmi_c.WmiMonitorBrightness():
    ...
    def _set_windows_brightness(self) -> Callable[[int], None]:
        ...
        def set_brightness(brightness):
            with com_initialized():
                ...
                monitor_names = []
                try:
                    for monitor in wmi_c.WmiMonitorBasicDisplayParams():
                ...
                success_count = 0
                for idx, monitor in enumerate(wmi_c.WmiMonitorBrightnessMethods()):
                    # 获取当前显示器名称
                    monitor_name = (
